...
#    idx = (np.abs(z - radius/2)).argmin() #find index of closest point to 1 Rj
    FAC = 1e-11 *(A[2]/A) # 1-7 microAm-1 from Ray+ 2009 # 7 for auroral, 3 for run 1
    b_temp = 700 #700 for run 1, 200 for subauroral and nonauroral, 2000 for auroral
    ...
    n_H_plus[2:-2,0] = 1*10**10*np.exp(-0.5*(z/z[0])) + n_H_plus_floor #2*10**9 (subauroral, run 1) #1*10**10 (auroral), #5*10**8 (nonauroral)
    n_H_plus[0:2,0] =1*10**10*np.exp(-0.5*(gb_z/z[0])) + n_H_plus_floor
    n_H_plus[-2:,0]=1*10**10*np.exp(-0.5*(ge_z/z[0]))+ n_H_plus_floor
    rho_H_plus[2:-2,0] = n_H_plus[2:-2,0] * m_H_plus 
    rho_H_plus[0:2,0]=n_H_plus[0:2,0] * m_H_plus
    rho_H_plus[-2:,0]=n_H_plus[-2:,0] * m_H_plus
    ...
    n_H3_plus[2:-2,0] = 5*10**10*np.exp(-0.4*(z/z[0])) + n_H3_plus_floor #1*10**10 (subauroral, run 1) #5*10**10 (auroral) #1*10**9 (nonauroral)
    n_H3_plus[0:2,0]=5*10**10*np.exp(-0.4*(gb_z/z[0])) + n_H3_plus_floor
    n_H3_plus[-2:,0]=5*10**10*np.exp(-0.4*(ge_z/z[0])) + n_H3_plus_floor
    rho_H3_plus[2:-2,0] = n_H3_plus[2:-2,0] * m_H3_plus
    rho_H3_plus[0:2,0]=n_H3_plus[0:2,0] * m_H3_plus
    rho_H3_plus[-2:,0]=n_H3_plus[-2:,0] * m_H3_plus
    ...
    # ----------NEUTRAL DENSITY-------------
    #initial neutral H2 density - CONSTANT
    n_H2[2:-2] = 10e16 *np.exp(-0.3*(z/z[0])) + 1000000#7 should be 6 #+ 0(10**10)*10**6
    n_H2[0:2]=10e16*np.exp(-0.3*(gb_z/z[0])) + 1000000
    n_H2[-2:]=10e16*np.exp(-0.3*(ge_z/z[0])) + 1000000
    rho_H2[2:-2] = n_H2[2:-2] * m_H2
    rho_H2[0:2]=n_H2[0:2] * m_H2
    rho_H2[-2:]=n_H2[-2:] * m_H2
    
    #initial neutral H density - CONSTANT
    n_H[2:-2] = 10e15*np.exp(-0.3*(z/z[0])) + 1000000#7 should be 6 #(10**9)*10**6
    n_H[0:2]=10e15*np.exp(-0.3*(gb_z/z[0])) + 1000000
    n_H[-2:]=10e15*np.exp(-0.3*(ge_z/z[0])) + 1000000
    rho_H[2:-2] = n_H[2:-2] * m_H
    rho_H[0:2]=n_H[0:2] * m_H
    rho_H[-2:]=n_H[-2:] * m_H
   
    #initial neutral He density - CONSTANT
    n_He[2:-2] = 10e14*np.exp(-0.5*(z/z[0])) + 1000000 #7 should be 6 #10**8*10**6
    n_He[0:2]=10e14*np.exp(-0.5*(gb_z/z[0])) + 1000000
    n_He[-2:]=10e14*np.exp(-0.5*(ge_z/z[0])) + 1000000
    rho_He[2:-2] = n_He[2:-2] * m_He
    rho_He[0:2]=n_He[0:2] * m_He
    rho_He[-2:]=n_He[-2:] * m_He
