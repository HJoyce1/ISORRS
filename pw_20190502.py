...
def polar_wind_outflow(planet_name,dt,its,rs,lshell,FAC_flag,CF_flag,plots,saves,run_name): 
    ...
    #folder = 'C:/Users/joyceh1/OneDrive - Lancaster University/pw_model/test_runs/easter/'
    ...
    x_is = np.linspace(73.01,76.99,5) # equally spaced in ionosphere
    ...
    #Iterate over ionospheric latitude for 100 field lines between the latitude limits
        ...
        #    rs = 3 #how far out in rs do you want to go - roughly?
            ...
            e_flux_tmp = np.empty([len(z)+4,its])
            ...
            ion_flux_tmp = np.empty([len(z_ext),its,num_ionic_species,])
            # ----------------------------------------------------------------------------   
            ...
            circ = 2*np.pi*(radius+25000000)*np.sin(np.deg2rad(14)) #auroral arc centred on 14deg
