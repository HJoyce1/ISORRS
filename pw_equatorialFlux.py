...
# OUTPUT: Plots for now, does not plot individual field lines, just final equatorial flux
#            expect run to take >hrs for larger iteration values
...
# =============================================================================
#polar_wind_outflow('jupiter',0.01,10000,3,0,0,1,0,'anything')
def polar_wind_outflow(planet_name,dt,its,rs,FAC_flag,CF_flag,plots,saves,run_name):  
    # ---------------------------------Imports-------------------------------------
    import numpy as np
    ...
    for h in range(1,len(x_is)+1):
        ...
        phi,ag,ac = dipolefield.dipolefield_carley(radius,inner,z[-1],mass_planet,b0,y_eq[h-1],rot_period,dipole_offset,numpoints)   
